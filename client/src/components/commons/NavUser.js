...
export default function NavUser(props) {
  ...
  return (
    <nav style={{ zIndex: 1041 }} className="navbar fixed-top navbar-expand-xl navbar-dark shadow-sm py-0 px-2 navUser">
            ...
            {appState.categories.data
              ? appState.categories.data.map((item, index) => (
                  <li key={index} className="nav-item">
                    <Link className="text-color nav-link font-weight" to={`/category/${item.name && hanldeUrlPretty(item.name)}/${item._id}`}>
