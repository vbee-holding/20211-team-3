...
export default function NewsDetail(props) {
  ...
  const increaseViewAndCheckIp = async (id, createdBy) => {
  ...
  React.useEffect(() => {
    ...
    const increaseViewWithoutCheckingIP = async (id, createdBy) => {
      // thống kê lượt xem
      axios.post("/statisticals/news", { id: id, createdBy: createdBy._id });
      const resIncreaseViews = await axios.put(`/news/views/${id}`, {
        views: props.datas.view + 1
    ...
    if (props?.datas?._id) {
      ...
      increaseViewWithoutCheckingIP(id, createdBy)
  ...
  const handleDelete = async (commentId) => {
    try {
      const res = await axios.delete(`/comments`, { params: { commentId: commentId, newsId: props.datas._id } });
  ...
  return (
    <React.Fragment>
      <Message />
      {datas ? datas.content === ""
        ? (<RedirectRoute to={datas.originalLink} />) : (
