        ...
        ? datas.map((data, index) => (
            <Link to={`/${hanldeUrlPretty(data.title)}/${data._id}`} key={index} className="other-new p-3 bg-white rounded text-decoration-none">
              ...
              <div className="other-new__info">
                ...
                <p className="other-new__createby text-secondary"><i className="mdi mdi-monitor" /> {data.createdBy === null ? "": data.createdBy.username} - <i className="mdi mdi-av-timer" /> {moment(data.dateCreate).format("DD-MM-YYYY")} - <i className="mdi mdi-eye" /> {data.view}</p>
              </div>
            </Link>
          ))
        : (<BoxLoadingItem />)}
